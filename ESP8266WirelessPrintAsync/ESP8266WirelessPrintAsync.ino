...
Temperature parseTemp(const String response, const String whichTemp) {
  ...
  if (tpos != -1) { // This response contains a temperature
    ...
    if (spacepos - tpos < 17) {
      temperatureRecord.actual = response.substring(tpos + whichTemp.length() + 1, slashpos);
      temperatureRecord.target = response.substring(slashpos + 2, spacepos);
    }
  }
...
bool parseTemperatures(const String response) {
  ...
  if (fwExtruders == 1) {
    parsedTemp = parseTemp(response, "T");
    if (parsedTemp.actual != "") {
      toolTemperature[0] = parsedTemp;
      tempResponse = true;
    }
  }
  else {
    for (int t = 0; t < fwExtruders; t++) {
      String toolTemperatureID = "T" + String(t);

      parsedTemp = parseTemp(response, toolTemperatureID);
      if (parsedTemp.actual != "") {
        toolTemperature[t] = parsedTemp;
        tempResponse = true;
      }
    }
  }

  parsedTemp = parseTemp(response, "B");
  if (parsedTemp.actual != "") {
    bedTemperature = parsedTemp;
    tempResponse = true;
  }
...
void handlePrint() {
  ...
  if (!isPrinting && (startPrint || restartPrint)) {
    ...
    gcodeFile = FileWrapper::open(uploadedFullname, "r");
    if (!gcodeFile)
      lcd("Can't open file");
    else {
      lcd("Printing...");
...
void handleUpload(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final) {
  ...
  if (!index) {
    if (uploadedFullname != "")
      file.remove(uploadedFullname);    // Remove previous file
    int pos = filename.lastIndexOf("/");
    uploadedFullname = pos == -1 ? "/" + filename : filename.substring(pos);
    if (uploadedFullname.length() > maxPathLength)
      uploadedFullname = "/cached.gco";   // TODO maybe a different solution
    file = FileWrapper::open(uploadedFullname, "w"); // create or truncate file
    }  
...
int apiPrinterCommandHandler(const char * data) {
...
int apiJobHandler(const char * data) {
  DynamicJsonBuffer jsonBuffer;
  JsonObject& root = jsonBuffer.parseObject(data);
    ...
    case 20:
      ...
      if (commandQueue.isEmpty()) {
        ...
        if (value == "") {
          ...
          mDNSInit();
          String text = IpAddress2String(WiFi.localIP()) + (hasSD ? " SD" : " SPIFFS");
          lcd(text);
          playSound();
...
inline void initSPIFFS() {
  ...
  if (dir.next()) {
    uploadedFullname = dir.fileName();
    uploadedFileSize = dir.fileSize();
  }
...
void setup() {
  ...
  #ifdef USE_FAST_SD
    hasSD = SD.begin(SD_CHIP_SELECT_PIN, 50000000); // https://github.com/esp8266/Arduino/issues/1853
  #else
    hasSD = SD.begin();
  #endif

  FileWrapper::begin(hasSD);
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest * request) {
    ...
    if (uploadedFullname != "") {
      message += "Uploaded file: " + getUploadedFilename() + "\n"
                 ...
                 "File lenght limit: " + String(maxPathLength) + "\n";
    }
    message += "Last command sent: " + lastCommandSent + "\n"
  ...
  server.onRequestBody([](AsyncWebServerRequest * request, uint8_t *data, size_t len, size_t index, size_t total) {
    ...
    if (request->url() == "/api/printer/command") {
      ...
      returnCode = apiPrinterCommandHandler((const char *) data);
    }
    else if (request->url() == "/api/job") {
      // http://docs.octoprint.org/en/master/api/job.html
      returnCode = apiJobHandler((const char *) data);
    }
    else
      returnCode = 204;
